package sample.compiler.core.code.analyser;
...
public class Generator {
	...
	private void handleFunc(Node func) {
		...
		// 函数开始，定义的四元式
		// fund 函数定义
		FourItem fund = new FourItem("fund", id, "", "");
		// 添加函数定义四元式
		if ("main".equals(id)){
			// main 函数不用添加
		}else{
	...
	private void handleItem(Node item) {
		...
		if (node.getName().equals("IDENTIFIER")) {
			...
			if (item.getSon(1).getSons().size() == 1) {
				...
				fourItem.result = this.newTempVar();
	...
	private void handleIfstatement(Node ifstatement) {
		...
		// 处理 else
		// 添加为假跳转
		FourItem jzItem = new FourItem("jz", "", "", ""+(this.fourItemList.size()+2));
